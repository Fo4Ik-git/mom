...
export function DynamicCalculator({ config }: DynamicCalculatorProps) {
  ...
  function runCalculation(nextQuantities: Record<string, number>) {
    try {
      const { values, warnings: nextWarnings } =
        calculateFromConfigWithDiagnostics(config, nextQuantities);
      setResults(values);
  ...
  const resultRows = useMemo(() => {
    ...
    const breakdowns = buildOutputBreakdowns(config, quantities, results);
  ...
  }, [config, config.outputs, quantities, results, warnings]);
  ...
  const handleClear = () => {
    const initial = buildInitialQuantities(config);
