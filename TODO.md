...
- Улучшение редактора калькулятора
  ...
  - Упрощение логики (много однотипных полей → сложно работать в редакторе)
    ...
    - **2. Агрегаты в языке формул (средняя сложность)**
      - [ ] Операнды-агрегаты: `SUM` по полям/calc/шаблону property (`var_cost`, `calc_field_*`, …)
      - [ ] При необходимости: `COUNT`, `AVG`, `MIN`, `MAX`
      - [ ] Палитра + eval (`block-evaluate`, `field-graph`) + валидация зависимостей
