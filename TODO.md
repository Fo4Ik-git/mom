...
- **ИИ-ассистент: учёт токенов, доступ и квоты**
  ...
  - **1. Учёт потраченных токенов**
    - [ ] Логировать каждый вызов `generate-calculator` / чат: `userId`, модель, `promptTokens`, `completionTokens`, `totalTokens`, timestamp, `calculatorId` (опционально)
    - [ ] Агрегаты в БД: сумма за всё время + за текущий период (день / неделя / месяц — см. п.3)
    - [ ] Брать цифры из ответа Gemini API (`usageMetadata`), не оценивать «на глаз»

  - **2. Доступ к ИИ для пользователя (выдаёт админ)**
    - [ ] Поля на пользователе (или отдельная сущность `AiAccess`):
      - включён / выключен
      - режим срока: **без срока** | **до даты** | **на N дней** с момента выдачи
    - [ ] UI в админке → модалка пользователя → раздел «ІІ-асистент» (рядом с рефералом / лимитами)
    - [ ] Проверка на API `POST /api/admin/ai/generate-calculator` и будущем user-facing endpoint: роль ADMIN **или** выданный доступ + не истёк срок
    - [ ] Админы по-прежнему без лимита токенов (или отдельная политика — зафиксировать в настройках)

  - **3. Лимит токенов на период**
    - [ ] Квота на пользователя: лимит + **период сброса** — день | неделя | месяц (пример: 100 000 / месяц)
    - [ ] `null` лимита = безлимит (для админа / VIP)
    - [ ] При исчерпании — 429 с понятным кодом (`ai_quota_exceeded`) и текстом «квота до {date}»
    - [ ] Опционально: дефолтная квота в platform settings для новых пользователей с доступом к ИИ

  - **4. Админ-панель — визуализация**
    - [ ] В таблице пользователей: колонка или бейдж «ІІ: X / Y за період» (потрачено / лимит)
    - [ ] В модалке пользователя → раздел «ІІ-асистент»:
      - потрачено за сегодня / неделю / месяц / всего
      - лимит и период, дата сброса
      - переключатели доступа и редактирование квоты
    - [ ] Опционально: график расхода за 30 дней на overview

  - **5. Интерфейс для пользователя**
    - [ ] Где показывать остаток: шапка билдера / панель ІІ-чату / «Мій акаунт» (выбрать одно место на MVP)
    - [ ] Текст вида: «Залишилось ~{remaining} токенів до {resetDate}»; при безлимите — не показывать или «без обмежень»
    - [ ] Если доступ не выдан или истёк — не показывать кнопку асистента (или заглушка «зверніться до адміна»)
